...
/**
 * Extension of StandardTokenizer that is aware of Wikipedia syntax. It is based
 * off of the Wikipedia tutorial available at
 * http://en.wikipedia.org/wiki/Wikipedia:Tutorial, but it may not be complete.
 * <p/>
 * <p/>
 * EXPERIMENTAL !!!!!!!!! NOTE: This Tokenizer is considered experimental and
 * the grammar is subject to change in the trunk and in follow up releases.
 */
public class CustomWikipediaTokenizer extends Tokenizer {
	...
	public static final String[] TOKEN_TYPES = new String[] { "<ALPHANUM>", "<APOSTROPHE>", "<ACRONYM>", "<COMPANY>",
			"<EMAIL>", "<HOST>", "<NUM>", "<CJ>", INTERNAL_LINK, EXTERNAL_LINK, CITATION, CATEGORY, BOLD, ITALICS,
	 ...
	 * {@link #TOKENS_ONLY} was used, produce multiple tokens.
	 */
	public static final int UNTOKENIZED_TOKEN_FLAG = 1;
	/**
	 * A private instance of the JFlex-constructed scanner
	 */
	private final CustomWikipediaTokenizerImpl scanner;
	 ...
	 *            The Input Reader
	 */
	public CustomWikipediaTokenizer(Reader input) {
		this(input, TOKENS_ONLY, Collections.EMPTY_SET);
	}

	@SuppressWarnings("unchecked")
	public CustomWikipediaTokenizer(Reader input, int tokenOutput, Set untokenizedTypes) {
		super(input);
		this.tokenOutput = tokenOutput;
		this.scanner = new CustomWikipediaTokenizerImpl(input);
	...
	public Token next(final Token reusableToken) throws IOException {
		...
		if (tokenType == CustomWikipediaTokenizerImpl.YYEOF) {
			return null;
		}
		String type = CustomWikipediaTokenizerImpl.TOKEN_TYPES[tokenType];
		if (tokenOutput == TOKENS_ONLY || untokenizedTypes.contains(type) == false) {
			...
			// collapse into a single token, add it to tokens AND output the
			// individual tokens
	...
	private void collapseAndSaveTokens(final Token reusableToken, int tokenType, String type) throws IOException {
		...
		while ((tmpTokType = scanner.getNextToken()) != CustomWikipediaTokenizerImpl.YYEOF && tmpTokType == tokenType
